...
export const getPublicComments = createAsyncThunk('ticket/getPublicComments', async (ticketId, thunkAPI) => {
  // console.log('public Comments in slice file');
  try {
...
export const getMyTickets = createAsyncThunk('tickets/getMyTickets', async (_, thunkAPI) => {
  try {
    return await TicketService.getTicketsForUser();
  } catch (error) {
    return thunkAPI.rejectWithValue(error.response?.data?.message || 'Failed to fetch user tickets');
  }
});
...
const ticketSlice = createSlice({
  ...
  reducers: {
    ...
    //     // Add to publicComments, ensure reference changes
    //     state.publicComments = [...state.publicComments, newComment]
    // },
    // addRealTimePublicComment: (state, action) => {
    //     const newComment = action.payload
    //     const commentExists = state.publicComments.some(
    ...
    addRealTimePublicComment: (state, action) => {
      const newComment = action.payload;
      const exists = state.publicComments.some((c) => c._id === newComment._id);
      if (!exists) {
        state.publicComments = [...state.publicComments, newComment];
      }
    },
    addRealTimePrivateComment: (state, action) => {
      const newComment = action.payload;
      state.privateComments = [...state.privateComments, newComment];
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getCustomerTickets.fulfilled, (state, action) => {
        ...
        // console.log('customerTickets in slice ', action.payload);
      })
      ...
      .addCase(deleteTicket.fulfilled, (state, action) => {
        ...
        toast.success('Ticket Deletd successfully!');
      })
      // Assign Ticket
      ...
      .addCase(getTicketById.fulfilled, (state, action) => {
        ...
        // if (action.payload.publicComments) {
      ...
      .addCase(addPublicComment.fulfilled, (state, action) => {
        state.isSendingComment = false;
        state.isTicketSuccess = true;
        // state.publicComments = [...state.publicComments, action.payload]
      ...
      .addCase(addPrivateComment.fulfilled, (state, action) => {
        ...
        toast.success('Messeage Sent "Internal');
      })
      ...
      .addMatcher(
        (action) => {
          const isTicket = action.type.startsWith('ticket/');
          const isComment = action.type.includes('addPublicComment') || action.type.includes('addPrivateComment');
          const isPending = action.type.endsWith('/pending');
          return isTicket && !isComment && isPending;
        },
        (state) => {
          state.isTicketLoading = true;
          state.isTicketError = false;
      ...
      .addMatcher(
        (action) => {
          ...
          return isTicket && !isComment && isRejected;
        },
        (state, action) => {
          state.isTicketLoading = false;
...
export const { resetTicketState, addRealTimePublicComment, addRealTimePrivateComment } = ticketSlice.actions;
